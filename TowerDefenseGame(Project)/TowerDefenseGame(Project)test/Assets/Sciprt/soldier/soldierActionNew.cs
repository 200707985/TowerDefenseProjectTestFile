...
public class soldierActionNew : MonoBehaviour
{
    bool walk = true;
    bool fire = false;
    GameObject[] enemies;
    ...
    void UpdateTarget() {   // 更新最近的enemy為target 
        enemies = GameObject.FindGameObjectsWithTag("enemy");  // 找有enemy tag的enemy
    ...
    {
        walking();
        if (target == null) {   // 如果沒有找剽target, 不要做以下的
            return;
        }
